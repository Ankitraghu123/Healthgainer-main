...
import { fetchHealthGainers, deleteHealthGainer, updateHealthGainer } from "@/redux/slices/healthGainerSlice";
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const formattedData = {
        ...
        prohibitions: editData.prohibitions.split(",").map(item => item.trim())
      };
      
      await dispatch(updateHealthGainer({ id: editData._id, data: formattedData })).unwrap();
      setEditData(null);
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-2xl font-semibold text-gray-800">Manage Products Tabs</h2>
          <p className="mt-1 text-sm text-gray-600">View, edit, and manage all health gainer products</p>
        </div>
                  ...
                  <tr key={gainer._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-gray-900">{gainer.product.name}</div>
                          <div className="text-sm text-gray-500">{gainer.product.discount}% off</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900 line-clamp-2">{gainer.description}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{gainer.product.mrp}.00</div>
                      <div className="text-sm text-gray-500">{(gainer.product.mrp * (1 - gainer.product.discount / 100)).toFixed(0)}.00</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${gainer.product.stock > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {gainer.product.stock > 0 ? `${gainer.product.stock} in stock` : 'Out of stock'}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity" aria-hidden="true">
              <div className="absolute inset-0 bg-gray-500 opacity-75" onClick={() => setViewData(null)}></div>
            </div>
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <div className="flex justify-between items-start">
                      <h3 className="text-lg leading-6 font-medium text-gray-900">{viewData.product.name}</h3>
                      <button
                        type="button"
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <img 
                          src={viewData.product.images[0] || "/placeholder-product.jpg"} 
                        ...
                        <div className="mt-4">
                          ...
                          <div className="mt-2">
                            <p className="text-sm text-gray-500">MRP: {viewData.product.mrp}.00</p>
                            <p className="text-sm text-gray-500">Discount: {viewData.product.discount}%</p>
                            <p className="text-lg font-semibold">
                              Final Price: {(viewData.product.mrp * (1 - viewData.product.discount / 100)).toFixed(0)}.00
                            </p>
                          </div>
                        </div>
                      </div>
                      <div className="h-96 overflow-x-scroll p-4">
                        <div className="mb-4 ">
                          ...
                          <p className="mt-1 text-sm text-gray-600">{viewData.description}</p>
                        </div>
                        <div className="mb-4">
                          <h4 className="font-medium text-gray-900">How to Use</h4>
                          <p className="mt-1 text-sm text-gray-600">{viewData.howToUse}</p>
                        </div>
                        <div className="mb-4">
                        ...
                        <div className="mb-4">
                          <h4 className="font-medium text-gray-900">Prohibitions</h4>
                          <ul className="mt-1 list-disc list-inside text-sm text-gray-600">
                      ...
                      <div className="mt-2 space-y-3">
                        {viewData.faq.map((faqItem, index) => (
                          <div key={index} className="p-3 bg-gray-50 rounded-lg">
                            <p className="font-medium text-gray-800">{faqItem.question}</p>
                            <p className="mt-1 text-sm text-gray-600">{faqItem.answer}</p>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {editData && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity" aria-hidden="true">
              <div className="absolute inset-0 bg-gray-500 opacity-75" onClick={() => setEditData(null)}></div>
            </div>
            <span className="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <div className="flex justify-between items-start">
                      <h3 className="text-lg leading-6 font-medium text-gray-900">Edit Health Gainer</h3>
                      <button
                        type="button"
                        className="text-gray-400 hover:text-gray-500"
                    ...
                    <form onSubmit={handleUpdate} className="mt-4 space-y-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Prohibitions (comma separated)</label>
                        <textarea
                          name="prohibitions"
