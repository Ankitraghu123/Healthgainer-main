...
import { FaStar, FaChevronLeft, FaChevronRight, FaShoppingCart, FaHeart } from "react-icons/fa";
...
export default function ProductDetailPage() {
    ...
    const handleAddToCart = () => {
        ...
        dispatch(addToCart({ userId, productId: product._id, variantId: selectedVariant._id, quantity }))
            .unwrap()
            .then(() => {
                            ...
                            <div className="flex gap-3 mt-4 overflow-x-auto py-2 scrollbar-hide">
                                {product.images?.map((img, index) => (
                                    <motion.div
                                        ...
                                        className={`flex-shrink-0 w-16 h-16 rounded-md overflow-hidden cursor-pointer border-2 transition ${currentImageIndex === index ? 'border-blue-500' : 'border-transparent'}`}
                                    >
                                        <img
                                            src={img}
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex items-center mb-4">
                                    <div className="flex text-yellow-400 mr-2">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <FaStar key={star} className={star <= 4 ? 'fill-current' : 'text-gray-300'} />
                                        ))}
                                    </div>
                                ...
                                <div className="border-t border-gray-200 pt-4">
                                    ...
                                    <ul className="space-y-2">
                                        ...
                                        <li className="flex items-start">
                                            <span className="text-green-500 mr-2">✓</span>
                                            <span className="text-gray-700">Authentic product guarantee</span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
